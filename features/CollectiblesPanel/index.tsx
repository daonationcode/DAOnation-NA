...
const CollectiblesPanel = () => {
  ...
  return (
    <Card>
      <div className="w-full flex flex-wrap gap-6">{mockNFTs && mockNFTs.length > 0 ? mockNFTs.map((nft, i) => <NFTCard className="!w-2/4 !max-w-[300px]" item={nft} key={i} display={true} />) : <>You don't have activity yet</>}</div>
    </Card>
  );
};
